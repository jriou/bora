...
kable(nn,caption="Posterior distributions of the parameters (\\textit{Eff. n} means effective sample size; $\\hat{R}$ means the Gelman-Rubin convergence diagnostic)",escape=F)
```


```{r fig3, fig.height=2, fig.width=4,fig.cap="Posterior distributions for $R_0$ (panel A) and $\\rho$ (panel B). The dashed lines represent the corresponding prior distributions."}
