...
From the posterior estimates on $R_0$, $\rho$ and $\phi$ and the observed incidence, the future incidence of the outbreak was forecasted for the 104 following weeks. The forecast was based on a simulation of __`r fit$nchains*(fit$nit-fit$nwarmup)`__ stochastic epidemic trajectories. The mean predicted trajectory as well as the 50% and 95% prediction are presented in Fig. 4.
...
Several indicators of direct operationnal value were computed from the simulated epidemic trajectories.
...
ss = cbind(c("Total \\;observed \\;cases","Total overall cases","Final attack rate",""),
           rbind(ressum(fit$R_summarypred[1,],0),
                 ressum(fit$R_summarypred[2,],0),
                 ressum(fit$R_summarypred[3,],2),
                 ressum(fit$R_start,0),
                 ressum(fit$R_peak,0),
                 ressum(fit$R_end,0)
           )
)
